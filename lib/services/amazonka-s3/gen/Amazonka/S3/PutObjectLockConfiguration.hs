...
newPutObjectLockConfiguration pBucket_
  = PutObjectLockConfiguration'{checksumAlgorithm =
                                  Prelude.Nothing,
                                contentMD5 = Prelude.Nothing,
                                expectedBucketOwner = Prelude.Nothing,
                                requestPayer = Prelude.Nothing,
                                objectLockConfiguration = Prelude.Nothing,
                                token = Prelude.Nothing, bucket = pBucket_}
...
instance Core.AWSRequest PutObjectLockConfiguration
         where
        type AWSResponse PutObjectLockConfiguration =
             PutObjectLockConfigurationResponse
        request srv
        ...
        response
          = Response.receiveEmpty
              (\ s h x ->
                 PutObjectLockConfigurationResponse' Prelude.<$>
                   (h Core..#? "x-amz-request-charged") Prelude.<*>
                     (Prelude.pure (Prelude.fromEnum s)))

instance Core.AWSService PutObjectLockConfiguration
         where
...
instance Core.ToElement PutObjectLockConfiguration
         where
...
instance Core.ToHeaders PutObjectLockConfiguration
         where
...
instance Core.ToQuery PutObjectLockConfiguration
         where
        toQuery
