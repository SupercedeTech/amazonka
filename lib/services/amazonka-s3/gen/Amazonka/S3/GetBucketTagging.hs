...
newGetBucketTagging pBucket_
  = GetBucketTagging'{expectedBucketOwner =
                        Prelude.Nothing,
                      bucket = pBucket_}
...
instance Core.AWSRequest GetBucketTagging where
        ...
        response
          = Response.receiveXML
              (\ s h x ->
                 GetBucketTaggingResponse' Prelude.<$>
                   (Prelude.pure (Prelude.fromEnum s)) Prelude.<*>
                     (x Core..@? "TagSet" Core..!@ Prelude.mempty
                        Prelude.>>= Core.parseXMLList "Tag"))

instance Core.AWSService GetBucketTagging where
...
newGetBucketTaggingResponse pHttpStatus_
  = GetBucketTaggingResponse'{httpStatus =
                                pHttpStatus_,
