...
-- distinguish which log files came from which bucket.
newLoggingEnabled
    :: Prelude.Text -- ^ 'targetBucket'
    -> Prelude.Text -- ^ 'targetPrefix'
    -> LoggingEnabled
newLoggingEnabled pTargetBucket_ pTargetPrefix_
  = LoggingEnabled'{targetGrants = Prelude.Nothing,
...
instance Core.FromXML LoggingEnabled where
        parseXML x
          = LoggingEnabled' Prelude.<$>
              (x Core..@? "TargetGrants" Core..!@ Prelude.mempty
