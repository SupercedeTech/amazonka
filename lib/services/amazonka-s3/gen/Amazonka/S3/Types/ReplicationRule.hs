...
newReplicationRule pStatus_ pDestination_
  = ReplicationRule'{sourceSelectionCriteria =
                     ...
                     priority = Prelude.Nothing, prefix = Prelude.Nothing,
                     deleteMarkerReplication = Prelude.Nothing,
                     status = pStatus_, destination = pDestination_}
...
instance Core.FromXML ReplicationRule where
        parseXML x
          = ReplicationRule' Prelude.<$>
              (x Core..@? "SourceSelectionCriteria") Prelude.<*>
                (x Core..@? "ID")
        ...
        toXML ReplicationRule'{..}
          = Prelude.mconcat
              ["SourceSelectionCriteria" Core.@=
                 sourceSelectionCriteria,
               "ID" Core.@= id,
               "ExistingObjectReplication" Core.@=
                 existingObjectReplication,
               "Filter" Core.@= filter',
               "Priority" Core.@= priority, "Prefix" Core.@= prefix,
               "DeleteMarkerReplication" Core.@=
