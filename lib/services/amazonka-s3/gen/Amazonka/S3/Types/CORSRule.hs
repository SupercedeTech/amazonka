...
newCORSRule
    :: CORSRule
newCORSRule
  = CORSRule'{allowedHeaders = Prelude.Nothing,
              id = Prelude.Nothing,
...
instance Core.FromXML CORSRule where
        parseXML x
          = CORSRule' Prelude.<$>
              (Core.may (Core.parseXMLList "AllowedHeader") x)
                Prelude.<*> (x Core..@? "ID")
                Prelude.<*>
                (Core.may (Core.parseXMLList "ExposeHeader") x)
...
instance Core.ToXML CORSRule where
        toXML CORSRule'{..}
          = Prelude.mconcat
              [Core.toXML
                 (Core.toXMLList "AllowedHeader" Prelude.<$>
                    allowedHeaders),
               "ID" Core.@= id,
               Core.toXML
                 (Core.toXMLList "ExposeHeader" Prelude.<$>
                    exposeHeaders),
