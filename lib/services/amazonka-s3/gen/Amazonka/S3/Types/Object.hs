...
newObject
    :: ETag -- ^ 'eTag'
    -> Prelude.Integer -- ^ 'size'
    -> ObjectKey -- ^ 'key'
    -> ObjectStorageClass -- ^ 'storageClass'
    -> Prelude.UTCTime -- ^ 'lastModified'
    -> Object
newObject pETag_ pSize_ pKey_ pStorageClass_
  pLastModified_
  = Object'{checksumAlgorithm = Prelude.Nothing,
            owner = Prelude.Nothing, eTag = pETag_,
            size = pSize_, key = pKey_,
...
instance Core.FromXML Object where
        parseXML x
          = Object' Prelude.<$>
              (Core.may (Core.parseXMLList "ChecksumAlgorithm") x)
