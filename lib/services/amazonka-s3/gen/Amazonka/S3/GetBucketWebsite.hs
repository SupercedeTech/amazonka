...
newGetBucketWebsite pBucket_
  = GetBucketWebsite'{expectedBucketOwner =
                        Prelude.Nothing,
                      bucket = pBucket_}
...
instance Core.AWSRequest GetBucketWebsite where
        ...
        response
          = Response.receiveXML
              (\ s h x ->
                 GetBucketWebsiteResponse' Prelude.<$>
                   (x Core..@? "RoutingRules" Core..!@ Prelude.mempty
                      Prelude.>>=
                      Core.may (Core.parseXMLList "RoutingRule"))
...
newGetBucketWebsiteResponse pHttpStatus_
  = GetBucketWebsiteResponse'{routingRules =
                                Prelude.Nothing,
