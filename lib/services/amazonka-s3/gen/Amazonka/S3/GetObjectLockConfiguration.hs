...
newGetObjectLockConfiguration pBucket_
  = GetObjectLockConfiguration'{expectedBucketOwner =
                                  Prelude.Nothing,
                                bucket = pBucket_}
...
instance Core.AWSRequest GetObjectLockConfiguration
         where
        type AWSResponse GetObjectLockConfiguration =
             GetObjectLockConfigurationResponse
        request srv
        ...
        response
          = Response.receiveXML
              (\ s h x ->
                 GetObjectLockConfigurationResponse' Prelude.<$>
                   (Core.parseXML x) Prelude.<*>
                     (Prelude.pure (Prelude.fromEnum s)))

instance Core.AWSService GetObjectLockConfiguration
         where
...
instance Core.ToHeaders GetObjectLockConfiguration
         where
...
instance Core.ToQuery GetObjectLockConfiguration
         where
        toQuery
