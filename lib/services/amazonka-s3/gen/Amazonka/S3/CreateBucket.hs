...
newCreateBucket pBucket_
  = CreateBucket'{grantWriteACP = Prelude.Nothing,
                  ...
                  grantReadACP = Prelude.Nothing, bucket = pBucket_}
...
instance Core.AWSRequest CreateBucket where
        ...
        response
          = Response.receiveEmpty
              (\ s h x ->
                 CreateBucketResponse' Prelude.<$>
                   (h Core..#? "Location") Prelude.<*>
        ...
        toHeaders CreateBucket'{..}
          = Prelude.mconcat
              ["x-amz-grant-write-acp" Core.=# grantWriteACP,
               ...
               "x-amz-acl" Core.=# acl,
               "x-amz-bucket-object-lock-enabled" Core.=#
...
newCreateBucketResponse pHttpStatus_
  = CreateBucketResponse'{location = Prelude.Nothing,
