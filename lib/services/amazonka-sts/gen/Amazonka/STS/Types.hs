...
defaultService
  = Core.Service{Core._serviceAbbrev = "STS",
                 ...
                 Core._serviceError = Core.parseXMLError "STS",
                 Core._serviceRetry = retry}
  where retry
          = Core.Exponential{Core._retryBase = 5.0e-2,
                             Core._retryGrowth = 2, Core._retryAttempts = 5,
                             Core._retryCheck = check}
        check e
          | Lens.has (Core.hasStatus 429) e =
            Prelude.Just "too_many_requests"
          | Lens.has
              (Core.hasCode "RequestThrottledException" Prelude..
                 Core.hasStatus 400)
              e
          ...
          | Lens.has
              (Core.hasCode "Throttling" Prelude..
                 Core.hasStatus 400)
              e
          ...
          | Lens.has
              (Core.hasCode "IDPCommunicationError" Prelude..
                 Core.hasStatus 400)
              e
          ...
          | Lens.has
              (Core.hasCode "ThrottledException" Prelude..
                 Core.hasStatus 400)
              e
            = Prelude.Just "throttled_exception"
          | Lens.has
              (Core.hasCode "ThrottlingException" Prelude..
                 Core.hasStatus 400)
              e
...
_ExpiredTokenException
  = Core._MatchServiceError defaultService
