...
newGetSessionToken
    :: GetSessionToken
newGetSessionToken
  = GetSessionToken'{durationSeconds = Prelude.Nothing,
...
instance Core.AWSRequest GetSessionToken where
        ...
        response
          = Response.receiveXMLWrapper "GetSessionTokenResult"
              (\ s h x ->
                 GetSessionTokenResponse' Prelude.<$>
                   (x Core..@? "Credentials") Prelude.<*>
